...
def interleave_new_items(generator):
    ...
    def interleaving_fn(k, item_indices):
        # generator = Generator(seed=1234) # TODO change?
...
def transform_relative_to_ideal(train_results, metric_key, model_keys, absolute_measure=True):
    ...
    for model_key in model_keys:
        if model_key == "ideal" and not absolute_measure: # thesis:changed
            continue
...
def graph_metrics(train_results, metric_key, model_keys, label_map, mean_sigma=0, mult_sd=0, conf_sigma=0, ax=plt, colors=None):
    for m in model_keys:
        ...
        if colors != None:
            line = ax.plot(values, label=label_map[m], color=colors[m])
        else:
    ...
    ax.legend(facecolor='white', framealpha=1, loc='upper right', bbox_to_anchor=(1.7, 1.0))
    
def graph_metrics_loglog(train_results, metric_key, model_keys, label_map, mean_sigma=0, mult_sd=0, conf_sigma=0, ax=plt, colors=None):
    for m in model_keys:
        if not isinstance(train_results[metric_key][m], np.ndarray):
            train_results[metric_key][m] = np.array(train_results[metric_key][m])
        # average across trials and smooth, if necessary
        ...
        values = values[50:] # only take the results after start-up and train (needed for the loglog plots to be useful)
        if colors != None:
            line = ax.loglog(values, label=label_map[m], color=colors[m])
        else:
            line = ax.loglog(values, label=label_map[m])
        line_color = line[0].get_color()
        if mult_sd > 0:
            std = train_results[metric_key][m].std(axis=0)[50:] # again, ignore the first 50 timesteps
            timesteps = np.arange(len(std))
            low = values - mult_sd * std
            high = values + mult_sd * std
            if conf_sigma > 0:
...
def graph_relative_to_ideal(train_results, metric_key, model_keys, label_map, absolute_measure=True, mean_sigma=0, mult_sd=0, conf_sigma=0, ax=None):
    relative_dist = transform_relative_to_ideal(train_results, metric_key, model_keys, absolute_measure)
    graph_metrics(relative_dist, metric_key, model_keys, label_map, mean_sigma, mult_sd, conf_sigma, ax)
...
class IdealRecommender(ContentFiltering):
    ...
    # thesis:new
    def drift_users(self):
        # thesis:TODO write this smarter so you dont reuse code
        # update user profiles based on the attributes of items they interacted with
        ...
        # for the ideal recommender, update U hat to reflect the shifted preferences
        self.users_hat.value = self.actual_user_profiles

class TargetIdealRecommender(ContentFiltering):
    """
    With the Target Ideal Recommender, we make the *strong assumption* that the true scores are provided
    to the recommender system through a custom scoring function, which always returns the true
    underlying user-item scores. Therefore, this class is pretty much an empty skeleton; the only
    modification is that we don't update any internal state of the recommender at each time step.
    """
    ...
    def train(self):
        if hasattr(self, 'cumulative_interactions') and self.cumulative_interactions.sum() > 0: # if there are interactions present:
            ...
            for i in range(self.num_users):
                ...
                # thesis:new TO NORMALIZE USER PREFERENCES
                dist_sum = self.users_hat.value[i,:].sum()
                if(dist_sum < 0.98 or dist_sum > 1.1):
                    self.users_hat.value[i,:] = self.users_hat.value[i,:] / dist_sum

            # thesis:new if judgements are on then we incorporate them every time the model is trained.
            if self.judgements_on:
                judgements = self.users.get_preference_judgements()
                self.incorporate_preference_judgements(judgements, self.j_strength) # thesis:TODO: better place to put this?
        else:
