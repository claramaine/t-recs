...
class PredictedUserProfiles(Component):  # pylint: disable=too-many-ancestors
    ...
    def copy(self):
        """
        returns a copy of the predicted user profiles array
        """
...
class ActualUserProfiles(Component):  # pylint: disable=too-many-ancestors
    ...
    @property # thesis:changed, previous mistake
    def num_attrs(self):
        """
        Shortcut getter method for the number of attributes in each user profile.
        """
    ...
    def copy(self):
        """
        returns a copy of the actual user profiles array
        """
        return self.current_state.copy()
...
# thesis:new
class PreferenceJudgements(Component):
    """
    Preference judgements generated by the user and sent to the model.
    """

    def __init__(self, judgements=None, size=None, verbose=False, seed=None):
        self.name = "preference_judgements"
        Component.__init__(self, current_state=judgements, size=size, verbose=verbose, seed=seed)
    ...
    def append_judgements(self, new_judgements):
        """
        Appends a set of preference judgements for new users to the current set of preference judgements.
        ...
        new_scores: :obj:`numpy.ndarray` or :obj:`scipy.sparse.spmatrix`
            ...
            scores are being appended.
        """
        self.current_state = mo.vstack([self.current_state, new_judgements])
        
    def copy(self):
        """
        returns a copy of the preference judgements array
        """
        return self.current_state.copy()

# thesis:new
class TargetUserProfiles(Component):
    """
    Aspirational target user profiles, unknown to the model. This class is a container
    compatible with Numpy operations and it does not make assumptions on the
    size of the representation.
    """

    def __init__(self, target_user_profiles=None, size=None, verbose=False, seed=None):
        self.name = "target_user_profiles"
        Component.__init__(self, current_state=target_user_profiles, size=size, verbose=verbose, seed=seed)
    ...
    def append_new_users(self, new_user_targets):
        """
        Appends a set of new users (represented as some kind of matrix) to the current
        set of items. Assumes the new users have dimension :math:`|U_{new}|\\times|A|`,
        where :math:`U_{new}` indicates the number of new items to be appended.
        """
        self.current_state = mo.vstack([self.current_state, new_user_targets])
    # thesis:TODO: maybe a method for updating them? nonstationary? idk if that would be approporiate here
    
    def copy(self):
        """
        returns a copy of the target user profiles array
        """
        return self.current_state.copy()

class ExtendedUsers(BaseComponent):  # pylint: disable=too-many-ancestors
    """
    Class representing users in the system.

    # thesis:TODO modify this whole description
    ...
    ------------
        ...
        # thesis:new
        target_user_profiles: array_like, optional #thesis:TODO make it actually optional
            Representation of the target user profiles.
        ...
        # thesis:TODO
        preference_judgements: array_like, optional
        
        # thesis:TODO justify default value
        give_judgement_probability: float, default 1.0 
            The probability that any given user submits a new preference judgement
            at a timestep.
    ...
    ------------
        ...
        # thesis:new
        target_user_profiles: :obj:`numpy.ndarray`
            A matrix representing the *target* user profiles.
        ...
        # thesis:new
        preference_judgements: :obj:`numpy.ndarray`
            A :math:`|U|\\times|A|` matrix representing the difference between a user's target
            preference distribution and their actual preference distribution. # thesis:TODO finish description
        
        # thesis:TODO so give_judgement_probability is not an attribute? What is an attribute?
    ...
    --------
        ...
        ValueError
            ...
            #thesis:TODO also if target_user_profiles is none?
    """

    def __init__(
        self,
        actual_user_profiles=None,
        target_user_profiles=None, # thesis:new
        actual_user_scores=None,
        interact_with_items=None,
        preference_judgements=None, # thesis:new
        give_judgement_probability=1.0, # thesis:new
        size=None,
        num_users=None,
        drift=0,
        ...
        # thesis:TODO add error checking for target
        ...
        # thesis:TODO maybe generate target profiles if they're not passed?

        # check_consistency also returns num_items and num_attributes, which are not needed
        num_users = check_consistency( # thesis:TODO probably should check for consistency here
        ...
        if target_user_profiles is None and size is not None:
            row_zeros = np.zeros(size[1])  # one row vector of zeroes
            while target_user_profiles is None or mo.contains_row(target_user_profiles, row_zeros):
                # generate matrix until no row is the zero vector
                target_user_profiles = self.rng.normal(size=size)
        self.target_user_profiles = TargetUserProfiles(target_user_profiles) # thesis:new
        self.interact_with_items = interact_with_items
        
        # thesis:new if preference judgement vector is not given just generate that here
        if preference_judgements is None and size is not None:
            preference_judgements = self.generate_meta_preferences()
        self.preference_judgements = PreferenceJudgements(preference_judgements)
        
        # thesis:new
        if give_judgement_probability > 1 or give_judgement_probability < 0:
            raise ValueError("Creation probability cannot be less than 0 or greater than 1")
        self.give_judgement_probability = give_judgement_probability
    ...
    # thesis:new
    def get_target_user_scores(self, user=None):
        """
        Returns an array of target user scores.
        ...
        user: int or :obj:`numpy.ndarray` or list (optional, default: None)
            Specifies the user index (or indices) for which to return the
            target user scores. If ``None``, the function returns the whole
            matrix.
        ...
        """
        if user is None:
            return self.target_user_scores
        else:
            return self.target_user_scores[user, :]

    # thesis:new
    def generate_meta_preferences(self):
        """
        At each step, select which users give judgements to the recommender model based 
        on `give_judgement_probability`. Then, for users that will give judgements to the
        model, we randomly sample from a gamma distribution and mask for the values which are
        too small a judgement.
        """
        
        # thesis:TODO need to test in case arrays are None?
        
        # generate judgements by extracting the difference between the target user profile and the current profile
        meta_prefs = np.subtract(self.target_user_profiles.value, self.actual_user_profiles.value) # judgements close to 1 or -1: extreme, judgements close to 0: not mcuh

        return meta_prefs

    def get_preference_judgements(self, user=None): #thesis:new
        """
        Returns an array of user preference judgements
        ...
        user: int or :obj:`numpy.ndarray` or list (optional, default: None)
            Specifies the user index (or indices) for which to return the
           preference judgement array. If ``None``, the function returns the whole
            matrix.
        ...
        scores:
            An array of actual user scores for each item.
        """
        if self.preference_judgements is not None:
            return self.preference_judgements
        
        # thesis:TODO otherwise, generate them here.
        
        if user is None:
            return self.preference_judgements
        else:
            return self.preference_judgements[user, :]
    ...
    def update_profiles(self, item_attributes):
        ...
        #thesis:changed to users_hat from actual_user_profiles bc it was messing up drift with the target rec.
        
        drifted_prefs = mo.slerp(self.actual_user_profiles, item_attributes, perc=self.drift)
        if np.isnan(drifted_prefs.min()):
            raise ValueError("something wrong with the drifted profs, there be NaNs here.")

        self.actual_user_profiles.value = drifted_prefs
        
    # thesis:new
    def set_drift(self, new_drift):
        """
        Updates the drift value for this representation of users to a given value
        """
    ...
    def append_new_users(self, new_users, existing_items):
        ...
        where :math:`U_{new}` indicates the number of new items to be appended.
        """
        # thesis:TODO need to change this to also generate new target?
        ...
        self.actual_user_scores.store_state()
        # thesis: TODO add append_judgements? 

class DNUsers(ExtendedUsers):
    """
