...
def slerp(mat1, mat2, perc=0.05):
    ...
    row_dot_product[row_dot_product == 1.0] = 0.9999999999999999 
    # thesis:new protects against a super edge case from a rounding issue which results in a NaN later
    ...
    # if(sin_o == 0):
    #     raise ValueError("oh no a zero")
    
    num1 = np.divide(np.sin((1.0 - perc) * omega), sin_o) 
    num2 = np.divide(np.sin(perc * omega), sin_o)
    
    unit_rot = (
        num1 * mat1_norm.T
        + num2 * mat2_norm.T
    ).T 
    # thesis:TODO sometimes this returns NaN, i guess bc it divides by 0 somewhere?
    # specifically I think that sometimes sin_o rounds so much that we get functionally 0
    # and then that turns into 0/0 and then we get nan
    if np.isnan(np.array([unit_rot * mat1_length]).min()):
        raise ValueError("something wrong with the drifted profs, there be NaNs here.")
