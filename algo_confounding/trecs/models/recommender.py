...
class BaseRecommender(MeasurementModule, SystemStateModule, VerboseMode, ABC):
    ...
    def __init__(  # pylint: disable=R0913,R0912,R0915
        ...
        verbose=False,
        seed=None,
        judgements_on=False #thesis:new
    ):
        # Init logger
        ...
        # check users array
        if not is_valid_or_none(users, (list, np.ndarray, sp.spmatrix, ExtendedUsers)):
            raise TypeError("users must be array_like or Users")
        if users is None:
            shape = (self.users_hat.num_users, self.users_hat.num_attrs)
            self.users = ExtendedUsers(size=shape, num_users=num_users, seed=seed)
        if isinstance(users, (list, np.ndarray, sp.spmatrix)):
            # assume that's what passed in is the user's profiles
            self.users = ExtendedUsers(actual_user_profiles=users, num_users=num_users)
        if isinstance(users, ExtendedUsers):
            self.users = users
        ...
        self.judgements_on = judgements_on #thesis:TODO add this to docs
    ...
    # thesis:new
    def incorporate_preference_judgements(self, judgements, j_strength): # thesis:TODO dont know if judgements is gonna be an internal storage thing or a param
        """_summary_

        Args:
            judgements (_type_): _description_
            j_strength (_type_): _description_
        """
        
        # Version 1: when J is the difference from some target dist:
        U_hat_j = np.add(self.users_hat.value, judgements.value.dot(j_strength)) # thesis:TODO check this
        
        #thesis:new TO NORMALIZE USER PREFERENCES
        row_sums = U_hat_j.sum(axis=1)
        if j_strength > 1.0:
            row_sums = U_hat_j.sum(axis=1)
            U_hat_j = U_hat_j / row_sums[:, np.newaxis]
        
        self.users_hat.value = U_hat_j
    ...
    def run(
        ...
        for timestep in tqdm(range(timesteps), disable=disable_tqdm):
            ...
            if self.users.drift > 0 and not startup:
                self.drift_users() # thesis:changed
            # train between steps:
