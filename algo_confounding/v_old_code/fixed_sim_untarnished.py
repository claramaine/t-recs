...
from trecs.metrics import InteractionSimilarity, RMSEMeasurement, Measurement
...
from replication_sim import NewItemFactory
from udpf_utils import PredictedTargetSimilarity, ActualTargetSimilarity
import argparse
import os
import errno
import warnings
...
def calc_startup_rec_size(args):
    ...
    if args["repeated_training"]:
        ...
        total_items_in_startup = (args["num_items"]) * args["startup_iters"]
...
def run_ideal_sim(user_prefs, target_prefs, item_attrs, metrics, args, rng): #thesis:changed
    ...
    # u.set_drift(0.0) # need to set the drift to 0 bc this is the ideal sim (?)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default="t-recs/algo_confounding/udps_ouput")
    parser.add_argument('--startup_iters', type=int, default=50)
    parser.add_argument('--sim_iters', type=int, default=100) #timesteps
    parser.add_argument('--num_sims', type=int, default=1)
    parser.add_argument('--metrics', nargs='+', default=["predicted_target_similarity", "actual_target_similarity", "rmse",  "actual_user_profiles", "target_user_profiles", "mean_item_dist"])
    parser.add_argument('--num_attrs', type=int, default=20)
    ...
    # run simulations
    model_keys = ["ideal", "target_ideal", "content_chaney", "content_chaney_j_1.0", "content_chaney_j_0.1", "random"]
    ...
    for i in range(args["num_sims"]):
        ...
        pairs = [rng.choice(args["num_users"], 2, replace=False) for _ in range(800)]
            # metrics = construct_metrics(["mean_item_dist", "interaction_history", "rmse"], pairs=pairs) # thesis:TODO add your metric here but also in construct_metrics
        metrics = construct_metrics(["interaction_history", "predicted_target_similarity", "actual_target_similarity", "actual_user_profiles", "target_user_profiles", "mean_item_dist"], pairs=pairs, target_prefs=target_prefs)
        ...
        metrics = construct_metrics(args['metrics'], pairs=pairs, ideal_interactions=ideal_interactions, ideal_item_attrs=ideal_attrs, target_prefs=target_prefs)
        models["content_chaney"] = run_content_sim(true_prefs, target_prefs, item_representation, metrics, args, rng)
        ...
        if args["judgements_on"] == True:
            ...
            args["alpha"] = 0.1
            metrics = construct_metrics(args['metrics'], pairs=pairs, ideal_interactions=ideal_interactions, ideal_item_attrs=ideal_attrs, target_prefs=target_prefs)
            models["content_chaney_j_0.1"] = run_content_sim(true_prefs, target_prefs, item_representation, metrics, args, rng, judgements_on=True)
            
            # metrics = construct_metrics(args['metrics'], pairs=pairs, ideal_interactions=ideal_interactions, ideal_item_attrs=ideal_attrs)
            # models["mf_j"] = run_mf_sim(true_prefs, item_representation, metrics, args, rng, judgements_on=True)
        
        # thesis:TODO actually run this when generating the results
        # metrics = construct_metrics(args['metrics'], pairs=pairs, ideal_interactions=ideal_interactions, ideal_item_attrs=ideal_attrs)
